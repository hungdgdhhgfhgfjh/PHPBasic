...
function searchbydate($customers,$from_date,$to_date){
    ...
    foreach($customers as $customer) 
    {
        ...
        continue;
        $fill_customers[]=$customers;
    }
    return $fill_customers;
}
$from_date=null;
$to_date=null;
if($_SEVER["REQUEST_METHOD"]=="POST"){
    $from_date=$_REQUEST["from"];
    $to_date=$_REQUEST["from"];
}
$fill_customers=searchbydate($customer_list,$from_date,$to_date)
?>
...
<table border ="1">
    ...
    <?php
        if (count($fill_customers) === 0 ):
    ?>
    <tr> <td colspan="5" class="message">Không tìm thấy nhân viên nào</td> </tr>
   <?php endif; ?>
   <?php foreach($fill_customers as $index => $customer): ?>
    <tr>
        <td><?php echo $index +1; ?> </td>
        <td><?php echo $customer['name']; ?> </td>
        <td> <?php echo $customer['Day Of Birth'];?></td>
